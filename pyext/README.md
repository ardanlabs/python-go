...
I love the interactive shell in Python, it let's me play around with code and
when I have a working version I write the code in a file.
...
You need to be extra careful when sharing memory between Go and Python (or C).
Sometimes it's not clear when does a memory allocation happen. For example, in
`export.go` line 13 we have the following code

```
str := C.CString(err.Error())
```

If you read `C.String` [documentation](https://golang.org/cmd/cgo/) you'll see
...
Lucky for us, Python is taking ownership on the allocated string and will free
...
Once we saw how the code works in the Python interactive prompt (aka
[REPL](https://en.wikipedia.org/wiki/Read%E2%80%93eval%E2%80%93print_loop)), we
can write a module to wrap the shared library.

**Listing 5: checksig.go**
```
...
06 
07 # Find out where the shared library is and
...
17 verify.restype = ctypes.c_char_p
18 
19 
20 class SignatureError(Exception):
21     """SignatureError is an error in signature"""
22     pass
23 
24 
25 def check_signature(root_dir):
26     """Check (in parallel) digital signature of all files in root_dir.
27     We assume there's a sha1sum.txt file under root_dir
28     """
29     res = verify(root_dir.encode('utf-8'))
30     if res is not None:
31         msg = res.decode('utf-8')
32         raise SignatureError(msg)
```

The code in lines 14-17 is very much what we did in the interactive prompt.
Lines 7-10 deal with the shared library name - we'll get to why we do that when
we'll talk about packaging below. Lines 20-22 define a custom exception type
for our package and in lines 25-32 we define the API of out module - a single
function called `check_signature`. ctypes will convert C's `NULL` to Python's
`None`, hence the `if` statement in line 30.

_Note: Python's naming conventions differ from Go. Most Python code is
...
defines the Python project, and then generate a `wheel` distribution and a
source (called `sdist`) distribution. If the user installs your package on a
machine that matches a wheel architecture - they'll get the binary package,
...
**Listing 8: Building the Packages**
```
...
**Listing 9: Content of `dist` Directory**
```
$ ls dist
...
With very little code, you can use Go from Python. Unlike the previous
installment, there's no RPC step - meaning you don't need to marshal and
unmarshal parameters on every function call and there's no network involved as
well. On the other hand you need to be more careful with memory management.

If you'd like to return Python types from Go (say a `list` or a `dict`), you
can use Python's [extensive C
API](https://docs.python.org/3/extending/index.html) using cgo. You can have a
look at the [go-python](https://github.com/sbinet/go-python) that can ease a
lot of the pain of writing Python extensions in Go.

In the next installment we're going to flip the roles again, we'll call Python
from Go - but this time, without RPC.
